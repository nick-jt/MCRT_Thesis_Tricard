...
These require extensive computational resources to model. Imposing the additional requirement of modeling radiation, which can be up of the same order of magnitude runtime as the CFD itself, is often-times infeasible. 

Several radiation models exist which promise accurate modeling at a reduced expense. Two of the most common methods are the method of spherical harmonics the discrete ordinates method (DOM).
These methods are often considered to be much faster, and offer very similar accuracy to the MCRT method under many circumstances. 
However, these methods are limited in that when they are supplied additional computational resources, they do not provide exact solutions. Monte-Carlo Ray Tracing, however, does offer this advantage~\cite{Leccese2018ConvectiveChambers}.
Additionally, while many consider Monte-Carlo ray tracing to be more computationally intensive, recent advanced implementations have shown that by using parallel processing, MCRT can consume even less runtime than its less accurate counterpart, DOM~\cite{Humphrey2016RadiativeRefinement}.
...
\caption{Angle definitions. $\theta$ is the polar angle, and $\psi$ is the azimuthal angle. }
